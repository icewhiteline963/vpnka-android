...
object YouTubeHistory {
    ...
    fun savePosition(url: String, posSec: Long, durationSec: Long) {
        ...
        // «Досмотрел» = добрался до последней десятой части ролика.
        //
        // Правило «последние 30 секунд» ломалось на обоих концах: у ролика
        // короче минуты оно было истинно при любом касании, а после защиты
        // «не короче минуты» короткие ролики нельзя было досмотреть вовсе.
        // Доля работает одинаково для пятиминутного и для сорокасекундного.
        val finished = durationSec > 0 && posSec >= durationSec * 0.9
