        ...
        webSocket = wsClient.newWebSocket(
            ...
            object : WebSocketListener() {
                ...
                override fun onFailure(ws: WebSocket, t: Throwable, r: okhttp3.Response?) {
                    ...
                    // ТОЛЬКО 403. 401 — это про сессию, а не про SmartDesk:
                    // протухший токен снимал бы разрешение, служба связи
                    // уходила бы, а обратно её никто не поднимал, потому что
                    // значение в профиле не менялось. Звонки просто
                    // переставали звонить до захода в «Сообщения».
                    if (r?.code == 403) {
