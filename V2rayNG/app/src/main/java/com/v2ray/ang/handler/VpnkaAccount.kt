...
object VpnkaAccount {
    ...
    data class Plan(
        ...
        // Per-plan traffic for the home-screen "X ГБ из N" bar, where N is
        // this limit (server-set, ~100 ГБ — never hard-coded). Limit is null
        // on paid plans (unlimited) — then no bar is shown.
        @SerializedName("traffic_used_bytes") val trafficUsedBytes: Long? = null,
    ...
    suspend fun smartDeskOnline(): Boolean = withContext(Dispatchers.IO) {
        ...
        // Напрямую, как остальной аккаунт-API (без прокси). Точка отражает
        // достижимость облака (get.vpnka.io — RU-edge), а не состояние туннеля.
