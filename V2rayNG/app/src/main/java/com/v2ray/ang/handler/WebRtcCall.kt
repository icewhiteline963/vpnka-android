...
object CallManager {
    ...
    private fun createPeer(context: Context, caller: Boolean) {
        ...
        if (!hasTurn) {
            ...
            // Собеседнику сообщаем, что разговора не будет: без этого его
            // телефон звонил до самого истечения ожидания, хотя на нашей
            // стороне уже всё решено.
            if (peerId != 0L) runCatching { signal(kind = "hangup") }
            cleanup(Phase.ENDED)
            return
        }
